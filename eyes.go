...
var glassRound = [3]string{"", "round", "square"}
...
func eyesToSVG(d diffs) (svg string) {
	...
	if d.eyes.glasses == noGlasses {
		...
		if d.eyes.glasses == roundGlasses {
			svg += fmt.Sprintf(`<circle cx="%d" cy="%d" r="%d"/>`, gx1, gy, gr)
			svg += fmt.Sprintf(`<circle cx="%d" cy="%d" r="%d"/>`, gx2, gy, gr)
		}
		if d.eyes.glasses == squareGlasses {
			svg += fmt.Sprintf(`<path d="M%d,%d l0,%d l%d,0 l0,%d Z"/>`, gx1-gr, gy-gr, 2*gr, 2*gr, -2*gr)
			svg += fmt.Sprintf(`<path d="M%d,%d l0,%d l%d,0 l0,%d Z"/>`, gx2-gr, gy-gr, 2*gr, 2*gr, -2*gr)
		}
